...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  if (normalized.includes('\\appdata\\local\\programs\\zoya\\resources')) {
    return 'user_local_programs';
  }
  if (
    normalized.includes('\\program files\\zoya\\resources') ||
    normalized.includes('\\program files (x86)\\zoya\\resources')
  ) {
