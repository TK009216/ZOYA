...
export async function getDefaultAionrsModel(): Promise<TProviderWithModel> {
  ...
  const savedDefault = configService.get('aionrs.globalDefault');
  if (savedDefault?.providerId && savedDefault.model) {
    const savedProvider = compatibleProviders.find((provider) => provider.id === savedDefault.providerId);
    if (savedProvider && getAvailableAionrsModels(savedProvider).includes(savedDefault.model)) {
      return {
        ...savedProvider,
        use_model: savedDefault.model,
      };
    }
  }
