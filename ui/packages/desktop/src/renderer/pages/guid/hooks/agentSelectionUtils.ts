...
/** Save preferred model ID to the agent's acp.config key and ZOYA config file */
export async function savePreferredModelId(agentKey: string, model_id: string): Promise<void> {
  try {
    ...
    // Also persist to ZOYA config file so new ACP sessions use this model
    if (agentKey === 'zoya' && model_id) {
      fetch('/api/zoya/config', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ model: model_id, small_model: model_id }),
      }).catch(() => {});
    }
  } catch {
    /* silent */
...
export async function saveAionrsDefaultModel(provider_id: string, use_model: string): Promise<void> {
  try {
    await configService.set('aionrs.globalDefault', { providerId: provider_id, model: use_model });
  } catch {
    /* silent */
  }
}
