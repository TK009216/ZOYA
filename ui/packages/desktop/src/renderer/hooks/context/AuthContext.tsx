...
const ZOYA_AUTH_KEY = 'zoya_authenticated';
...
export const AuthProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const refresh = useCallback(async () => {
    ...
    const zoomsAuth = (() => { try { return localStorage.getItem(ZOYA_AUTH_KEY) === 'true'; } catch { return false; } })();
    if (zoomsAuth) {
      setUser(null);
      setStatus('authenticated');
      setReady(true);
  ...
  const codeLogin = useCallback(async (): Promise<LoginResult> => {
    setUser(null);
    setStatus('authenticated');
    setReady(true);
    try { localStorage.setItem(ZOYA_AUTH_KEY, 'true'); } catch {}
    return { success: true };
  }, []);

  const logout = useCallback(async () => {
    try { localStorage.removeItem(ZOYA_AUTH_KEY); } catch {}
    if (isDesktopRuntime) {
      setUser(null);
      setStatus('authenticated');
      setReady(true);
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [codeLogin, login, logout, ready, refresh, status, user]
  );
