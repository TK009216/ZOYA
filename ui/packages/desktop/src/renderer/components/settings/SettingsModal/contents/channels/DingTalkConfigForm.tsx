...
const DINGTALK_DEV_DOCS_URL = 'https://zoya.local/wiki/DingTalk-Bot-Setup-Guide';
...
const DingTalkConfigForm: React.FC<DingTalkConfigFormProps> = ({ pluginStatus, modelSelection, onStatusChange }) => {
  ...
  const agentOptions: Array<{
    ...
    name: string;
    id?: string;
  }> = availableAgents.length > 0 ? availableAgents : [{ agent_type: 'aionrs', name: 'ZOYA CLI' }];
    ...
    <div className='flex flex-col gap-24px'>
      ...
      <div className='flex flex-col gap-8px py-12px'>
        <div className='text-14px font-500 text-t-primary'>
          {t('settings.assistant.defaultModel', 'Model')}
        </div>
        <div className='text-12px text-t-tertiary'>
          {t('settings.dingtalk.defaultModelDesc', 'Used for Agent conversations')}
        </div>
        <div className='text-14px text-t-primary italic mt-4px'>
