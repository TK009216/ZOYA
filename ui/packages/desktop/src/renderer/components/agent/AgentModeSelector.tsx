...
export interface AgentModeSelectorHandle {
  /** Apply any queued mode change (call when AI processing completes) */
  applyQueuedMode: () => void;
  /** Get the currently queued mode (null if none) */
  getQueuedMode: () => string | null;
}
...
export interface AgentModeSelectorProps {
  ...
  /** Disable mode switching (e.g. during AI reply processing) */
  disabled?: boolean;
  /** Callback when mode is queued during disabled state — fires AFTER processing completes */
  onQueuedModeChange?: (mode: string) => void;
  /** Externally set current mode (overrides internal state when provided) */
  externalMode?: string;
  /** Increment to force mode re-sync from backend (e.g. after popup mode change) */
  modeSyncKey?: number;
}
 ...
 */
const AgentModeSelector = React.forwardRef<AgentModeSelectorHandle, AgentModeSelectorProps>(({
  backend,
  agent_name,
  agentLogo,
  agentLogoIsEmoji,
  ...
  // Sync with externalMode when provided (e.g. from parent processing queue)
  useEffect(() => {
    if (externalMode !== undefined && externalMode !== current_mode) {
      const valid = modes.some((m) => m.value === externalMode);
      if (valid) {
  ...
  }, [conversation_id, can_switchMode, beforeRuntimeSync, modeSyncKey]);
  ...
  const handleModeChange = useCallback(
    async (mode: string) => {
      ...
      // If disabled (e.g. AI is processing), queue the mode change
      if (disabled) {
        setQueuedMode(mode);
        Message.info(t('agentMode.modeQueued', { defaultValue: 'Mode change will apply after reply completes' }));
        return;
      }
    ...
    [backend, beforeRuntimeSync, conversation_id, current_mode, onModeChanged, onModeSelect, persistGlobalPreference, t, disabled]
  );

  const queuedModeRef = useRef<string | null>(null);
  // Keep ref in sync with state
  useEffect(() => { queuedModeRef.current = queuedMode; }, [queuedMode]);

  // Expose applyQueuedMode to parent via ref-like callback
  const applyQueuedModeInternal = useCallback(() => {
    const mode = queuedModeRef.current;
    if (!mode || !conversation_id) return;
    setQueuedMode(null);
    queuedModeRef.current = null;
    // Apply the queued mode change now that processing is done
    setIsLoading(true);
    void (async () => {
      try {
        await beforeRuntimeSync?.();
        ...
        setCurrentMode(confirmedMode);
        onModeChanged?.(confirmedMode);
        onQueuedModeChange?.(confirmedMode);
        if (backend && persistGlobalPreference) {
          void savePreferredMode(backend, confirmedMode);
        }
        Message.success(t('agentMode.switchSuccess'));
      } catch (error) {
        console.error('[AgentModeSelector] Failed to apply queued mode:', error);
        Message.error(t('agentMode.switchFailed'));
      } finally {
        setIsLoading(false);
      }
    })();
  }, [conversation_id, beforeRuntimeSync, onModeChanged, onQueuedModeChange, backend, persistGlobalPreference]);

  useImperativeHandle(ref, () => ({
    applyQueuedMode: applyQueuedModeInternal,
    getQueuedMode: () => queuedModeRef.current,
  }), [applyQueuedModeInternal]);
  ...
  if (compact) {
    ...
    const compactContent = (
      <span data-testid='mode-selector' data-current-mode={current_mode} className='inline-flex'>
        <Button
          ...
          shape='round'
          size='small'
          onClick={canInteract && !isDisabled ? () => setDropdownVisible((visible) => !visible) : undefined}
          style={{
            opacity: isDisabled ? 0.5 : 1,
            transition: 'opacity 0.2s',
            cursor: canInteract && !isDisabled ? 'pointer' : 'not-allowed',
          }}
        >
          <span className='flex items-center gap-6px min-w-0 leading-none'>
            ...
            <MarqueePillLabel>
              {queuedMode
                ? `${compactLabel} → ${modes.find((m) => m.value === queuedMode)?.label || queuedMode}`
                : compactLabel}
            </MarqueePillLabel>
            {canInteract && !isDisabled && <Down size={12} className='text-t-tertiary shrink-0' />}
            {isDisabled && <span className='w-6px h-6px bg-orange-400 rounded-full animate-pulse ml-4px' />}
          </span>
        </Button>
      </span>
    );
    ...
    return (
      <Dropdown
        trigger='click'
        popupVisible={!isDisabled ? dropdownVisible : false}
        onVisibleChange={(visible) => !isDisabled && setDropdownVisible(visible)}
