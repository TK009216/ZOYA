...
import siderLogo from '@renderer/assets/logos/brand/logo_1.png';
...
const Layout: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      ...
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('zoya-open-update-modal', { detail: { source: 'tray' } }));
      }, 100);
    };
            ...
            >
              <ArcoLayout.Header
                className={classNames(
                  'flex items-center justify-center pt-12px pb-12px pl-16px pr-16px gap-12px layout-sider-header',
              ...
              >
                <div onClick={() => { onClick(); setLogoZoom(true); setTimeout(() => setLogoZoom(false), 600); }} className={classNames('layout-sider-header-logo', {
                  'flex flex-col items-center gap-6px': !collapsed,
                  'flex items-center justify-center': collapsed,
                })}>
                  <img
                    src={siderLogo}
                    alt='ZOYA'
                    className={classNames('object-contain zoya-logo', {
                      'size-120px': !collapsed,
                      'size-24px': collapsed,
                    }, { 'zoya-logo-zoom': logoZoom })}
                  />
                  <div className={classNames('zoya-logo-text', {
