...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  const feedbackTooltip = t('conversation.welcome.quickActionFeedback', { defaultValue: 'Feedback' });
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('zoya:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('zoya:last-non-settings-path');
      if (stored) {
