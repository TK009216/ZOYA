...
// Override via ZOYA_CDP_PORT env variable. Set to "0" to disable.
//
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another ZOYA instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active ZOYA instances, trying ${preferredPort}`
  );
  return preferredPort;
}
