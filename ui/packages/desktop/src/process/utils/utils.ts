...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'zoya');
};
 ...
 * Release builds use ~/.zoya; dev builds use ~/.zoya-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.zoya，Dev 模式使用 ~/.zoya-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'zoya');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.zoya'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.zoya-config; dev builds use ~/.zoya-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.zoya-config，Dev 模式使用 ~/.zoya-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.zoya-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[ZOYA] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[ZOYA] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[ZOYA] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[ZOYA] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
