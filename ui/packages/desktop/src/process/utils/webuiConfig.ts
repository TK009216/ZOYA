...
import { startWebHost, type WebHostHandle } from '@zoya/web-host';
...
const DEFAULT_WEBUI_PORT = (() => {
  ...
  if (process.env.zoya_MULTI_INSTANCE === '1') return 25810;
  return 25809;
})();
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.zoya_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.zoya_ALLOW_REMOTE || process.env.zoya_REMOTE);
  const hostHint = process.env.zoya_HOST?.trim();
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.zoya[-dev] symlink on macOS to sidestep
