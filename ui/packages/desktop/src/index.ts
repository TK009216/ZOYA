...
import { BackendLifecycleManager } from '@zoya/web-host';
...
const isE2ETestMode = process.env.ZOYA_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.ZOYA_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[ZOYA] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[ZOYA] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[ZOYA] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[ZOYA] runBackendMigrations completed');
    } catch (error) {
      console.error('[ZOYA] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[ZOYA] ${source} ready (port=${backendPort})`);
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[ZOYA] Creating main window...');
  ...
  console.log(`[ZOYA] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[ZOYA] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[ZOYA] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[ZOYA] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  const disableAutoUpdater =
    process.env.ZOYA_DISABLE_AUTO_UPDATE === '1' || process.env.ZOYA_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[ZOYA] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[ZOYA] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[ZOYA] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[ZOYA] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[ZOYA] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[ZOYA] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[ZOYA] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[ZOYA] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[ZOYA] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[ZOYA] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[ZOYA] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[ZOYA] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[ZOYA] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[ZOYA:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[ZOYA] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[ZOYA] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`ZOYA --webui` or packaged `aionui-web` mode that
      ...
      const handle = await startWebHost({
        ...
        // Expose the same ZOYA_{CACHE,WORK,LOG}_DIR env the desktop IPC path
...
// Register ZOYA:// as the default protocol client
...
// macOS: handle ZOYA:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[ZOYA] App initialization failed:', error);
    app.quit();
  });
...
app.on('will-quit', () => {
  console.log('[ZOYA] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[ZOYA] quit (exitCode=${exitCode})`);
});
