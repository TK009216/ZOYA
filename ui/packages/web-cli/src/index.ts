import { startWebHost, startStaticServer } from '@zoya/web-host';
import type { WebHostHandle, StaticServerHandle } from '@zoya/web-host';
...
// tarball layout:
//   zoya-web/
//   ├── zoya-web              ← bun-compiled standalone binary (process.execPath)
...
const isPackaged = (() => {
  ...
  return exeName === 'zoya-web' || exeName === 'zoya-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'aioncore.exe' : 'aioncore';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'zoya-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.ZOYA_BACKEND_BIN;
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.ZOYA_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.zoya-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.ZOYA_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.ZOYA_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.ZOYA_ALLOW_REMOTE ?? process.env.ZOYA_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[zoya-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[zoya-web] version    : ${version}`);
  console.log(`[zoya-web] data dir   : ${dataDir}`);
  console.log(`[zoya-web] log dir    : ${logDir}`);
  console.log(`[zoya-web] static dir : ${staticDir}`);
  console.log(`[zoya-web] backend bin: ${backendBin}`);
  console.log(`[zoya-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download aioncore and set ZOYA_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('ZOYA WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[zoya-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[zoya-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[zoya-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[zoya-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `zoya-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[zoya-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set ZOYA_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[zoya-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[zoya-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[zoya-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[zoya-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[zoya-web] username: ${username}`);
    console.log(`[zoya-web] new password: ${newPassword}`);
    console.log('[zoya-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: zoya-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.zoya-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.zoya-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  ZOYA_PORT, ZOYA_ALLOW_REMOTE, ZOYA_DATA_DIR, ZOYA_LOG_DIR,
  ZOYA_BACKEND_BIN, ZOYA_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: zoya-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[zoya-web] fatal:', err.message);
