...
var __electron_vite_injected_dirname = "D:\\PROJECTS\\ZOYA_009\\ui\\packages\\desktop";
  ...
  return {
    main: {
      plugins: [
        ...
        // '@zoya/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@zoya/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ["fix-path", "@zoya/web-host"] }),
        ...isDevelopment ? [
          {
      ...
      plugins: [
        ...
        ...!isDevelopment ? [
          viteStaticCopy({
            ...
            environment: "client",
            targets: [
              { src: "assets/logos/**", dest: "assets/logos" }
            ]
          })
        ] : [],
        ...enableSentrySourceMaps ? [sentryVitePlugin(sentryPluginOptions)] : []
