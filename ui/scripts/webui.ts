 ...
 *
 * Pure Bun CLI â€” launches the WebUI (backend + static server + auth) without
 ...
 *
 * `bun run webui` runs **independently of the Electron desktop app** â€” it must
 ...
 * gemini, qwenâ€¦) don't choke on the literal space in "Application Support".
 ...
 * falls back to returning the space-containing path â€” and then every ACP
 ...
 * spawned by ACP â€” the `Superset: X not found in PATH` wrapper bails, so the
 ...
 * fallback â€” useful when the backend is unreachable or the SQLite users row
...
async function main(): Promise<void> {
  ...
  // for the Electron path â€” SQLite is now the single source of truth.
  //
  ...
  // Ensure ZOYA is registered as a detected (builtin) agent
  async function ensureZoyaAgent(): Promise<void> {
    const api = (path: string) => `http://127.0.0.1:${handle.backendPort}${path}`;

    // Find all existing ZOYA entries (dedup: skip if any exist)
    let existing: { id: string; agent_source: string } | undefined;
    try {
      const list = await (await fetch(api('/api/agents'))).json();
      const items = Array.isArray(list) ? list : (list as { data?: unknown[] }).data ?? [];
      const zoyaEntries = (items as Array<{ id: string; name: string; agent_source: string }>).filter(
        (a: { id: string; name: string; agent_source: string }) => a.name === 'ZOYA'
      );
      // Remove duplicates via API DELETE
      if (zoyaEntries.length > 1) {
        for (let i = 1; i < zoyaEntries.length; i++) {
          try { await fetch(api(`/api/agents/custom/${zoyaEntries[i].id}`), { method: 'DELETE' }); } catch { /* best effort */ }
        }
      }
      existing = zoyaEntries[0];
    } catch { /* ignore */ }

    if (existing) {
      console.log('[webui] ZOYA already registered, id=' + existing.id + ', source=' + existing.agent_source);
    }

    // Register via API only if no ZOYA exists at all
    let zoyaId = existing?.id;
    if (!existing) {
      try {
        const payload = {
          name: 'ZOYA',
          ...
          advanced: { yolo_id: 'build', behavior_policy: { supports_side_question: true } },
        };
        const reg = await fetch(api('/api/agents/custom'), {
          method: 'POST',
        ...
        if (reg.ok) {
          ...
          zoyaId = body.id;
          console.log('[webui] ZOYA agent registered via API, id=' + zoyaId);
        } else {
          console.log('[webui] ZOYA agent registration skipped:', reg.status);
        }
      } catch (err) {
        console.warn('[webui] could not register ZOYA agent:', err);
      }
    }

    // Look up ZOYA id if we still don't have it
    if (!zoyaId) {
      try {
        const list = await (await fetch(api('/api/agents'))).json();
        const items = Array.isArray(list) ? list : (list as { data?: unknown[] }).data ?? [];
        const match = (items as Array<{ id: string; name: string }>).find(
          (a: { id: string; name: string }) => a.name === 'ZOYA'
        );
        if (match) zoyaId = match.id;
      } catch { /* ignore */ }
    }
      ...
      try {
        ...
        if (fs.existsSync(dbPath)) {
          ...
          if (zoyaId && typeof zoyaId === 'string' && zoyaId.length > 0) {
            db.run("UPDATE agent_metadata SET agent_source = 'builtin', backend = 'zoya' WHERE id = ?", [zoyaId]);
          } else {
            console.warn('[webui] Skipping DB patch: invalid zoyaId received', zoyaId);
          }
          db.close();
          console.log('[webui] ZOYA DB entry promoted to builtin (detected)');
        }
      } catch (err) {
        console.warn('[webui] could not patch ZOYA DB entry:', err);
      }
    }

    // Tell backend to re-read agent_metadata from DB
    try {
      await fetch(api('/api/agents/refresh'), { method: 'POST' });
      console.log('[webui] agent cache refreshed');
    } catch (err) {
      console.warn('[webui] could not refresh agent cache:', err);
    }
  }

  await ensureZoyaAgent();
