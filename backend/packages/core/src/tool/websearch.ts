...
import { ToolFailure } from "@zoya/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@zoya/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ZOYA_WEBSEARCH_PROVIDER === "exa" || process.env.ZOYA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ZOYA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ZOYA_EXPERIMENTAL") || truthy("ZOYA_ENABLE_EXA") || truthy("ZOYA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ZOYA_ENABLE_PARALLEL") || truthy("ZOYA_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `zoya/${InstallationVersion}`,
