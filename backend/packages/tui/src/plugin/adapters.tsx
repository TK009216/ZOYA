import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@zoya/plugin/tui"
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getZoyaModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getZoyaModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
