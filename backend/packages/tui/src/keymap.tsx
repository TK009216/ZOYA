...
type ZoyaModeStack = ReturnType<typeof createZoyaModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ZoyaModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createZoyaModeStack(keymap: OpenTuiKeymap) {
...
export function useZoyaModeStack() {
  return getZoyaModeStack(useOpencodeKeymap())
}

export function getZoyaModeStack(keymap: OpenTuiKeymap) {
...
export function registerOpencodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createZoyaModeStack(keymap)
