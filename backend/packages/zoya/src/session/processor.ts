...
import { ToolOutput, Usage, type LLMEvent } from "@zoya/llm"
...
export class Service extends Context.Service<Service, Interface>()("@zoya/SessionProcessor") {}
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const cleanup = Effect.fn("SessionProcessor.cleanup")(function* () {
        ...
        // If the assistant has no text content and no tool calls but has
        // reasoning, use the reasoning text as fallback content. This handles
        // reasoning models (like deepseek-v4-flash-free) that sometimes burn
        // all output tokens on reasoning, leaving content empty.
        if (!ctx.currentText && Object.keys(ctx.toolcalls).length === 0 && reasoningParts.length > 0) {
