...
import { MessageTable, PartTable, SessionTable } from "@zoya/core/session/sql"
...
function providerMeta(metadata: Record<string, any> | undefined) {
  ...
  const { providerExecuted: _, providerOptions: __, ...rest } = metadata
  return Object.keys(rest).length > 0 ? rest : undefined
}

function sanitizeProviderMetadata(metadata: unknown): unknown {
  if (!metadata || typeof metadata !== "object") return metadata
  const clone = { ...metadata as Record<string, unknown> }
  // Provider-level metadata keys are provider names mapped to provider-specific
  // records. Strip anything that isn't shaped like that — stage/step are ZOYA's
  // internal pipeline tracking and must never be forwarded to the LLM.
  for (const key of Object.keys(clone)) {
    if (!isRecord(clone[key])) delete clone[key]
  }
      ...
      for (const part of msg.parts) {
        if (part.type === "text") {
          ...
          assistantMessage.parts.push({
            ...
            text,
            ...(differentModel || !meta ? {} : {
              providerMetadata: sanitizeProviderMetadata(meta),
            }),
          })
        }
        ...
        if (part.type === "reasoning") {
          ...
          assistantMessage.parts.push({
            type: "reasoning",
            text: part.text,
            providerMetadata: sanitizeProviderMetadata(part.metadata),
          })
        }
      }
