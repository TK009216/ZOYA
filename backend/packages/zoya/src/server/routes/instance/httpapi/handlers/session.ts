      ...
      const message = yield* promptSvc
        ...
        .pipe(
          Effect.tapError((error) => {
            console.error("[ZOYA_DEBUG] prompt error:", String(error))
            return Effect.logError("SessionHttpApi.prompt error", {
              sessionID: ctx.params.sessionID,
              error: String(error),
            })
          }),
          Effect.catchCause((cause) => {
            console.error("[ZOYA_DEBUG] prompt cause:", Cause.pretty(cause))
            return Effect.gen(function* () {
              yield* Effect.logError("SessionHttpApi.prompt cause", {
                sessionID: ctx.params.sessionID,
