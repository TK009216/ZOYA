...
import type * as ModelsDev from "@zoya/core/models-dev"
...
export function options(input: {
  ...
  // Ensure reasoning parameters are set if the model is reasoning-capable
  if (input.model.capabilities.reasoning && !result["reasoning"] && !result["reasoningEffort"]) {
    // DeepSeek/OpenAI-compatible uses reasoning_effort string, not reasoning object
    const npm = input.model.api.npm
    if (npm === "@ai-sdk/openai-compatible" || input.model.api.id.toLowerCase().includes("deepseek")) {
