...
export function toRequestError(error: Error) {
  switch (error._tag) {
    ...
    default:
      return RequestError.internalError({}, "Unknown error type")
  }
}

export function fromUnknownDefect(defect: unknown, safeMessage = "Internal service failure") {
  const message = defect instanceof Error ? defect.message : safeMessage
  return new ServiceFailureError({ safeMessage: message })
}
