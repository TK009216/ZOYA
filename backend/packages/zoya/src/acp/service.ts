...
import { InstallationVersion } from "@zoya/core/installation/version"
import type { Message, ZoyaClient, SessionMessageResponse } from "@zoya/sdk/v2"
...
import { ProviderV2 } from "@zoya/core/provider"
import { ModelV2 } from "@zoya/core/model"
...
export class Service extends Context.Service<Service, Interface>()("@zoya/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `zoya auth login` in the terminal",
      name: "Login with zoya",
      id: AuthMethodID,
    }
  ...
  const newSession = Effect.fn("ACP.newSession")(function* (params: NewSessionRequest) {
    ...
    if (!selected) {
      ...
      const providerList = Object.entries(snapshot.providers).map(([k, v]) => `${k}:${Object.keys(v.models).length}models`)
      console.error("ACP FATAL: no model selected. Providers:", providers, "providerDetails:", providerList, "effectiveModel:", snapshot.effectiveModel, "availableModes:", snapshot.availableModes.map(m => m.id))
      return yield* Effect.fail(new ACPError.InvalidModelError({ modelId: "none", providerId: undefined }))
    }
    ...
    // Mode-model mapping: default mode ka apna model/variant hota hai
    let effectiveModel = selected
    let effectiveVariant = variant
    if (modeId) {
      const mm = MODE_MODEL_MAP[modeId]
      if (mm) {
        const p = snapshot.providers[ProviderV2.ID.make(mm.providerID)]
        if (p && p.models[ModelV2.ID.make(mm.modelID)]) {
          effectiveModel = { providerID: ProviderV2.ID.make(mm.providerID), modelID: ModelV2.ID.make(mm.modelID) }
          effectiveVariant = mm.variant
        }
      }
    }
    ...
    if (modeId) SystemPrompt.setSessionMode(state.id, modeId)
  ...
  const loadSession = Effect.fn("ACP.loadSession")(function* (params: LoadSessionRequest) {
    ...
    if (state.modeId) SystemPrompt.setSessionMode(state.id, state.modeId)
    sessionSnapshots.set(state.id, snapshot)
  ...
  const setSessionConfigOption = Effect.fn("ACP.setSessionConfigOption")(function* (
    ...
    if (params.configId === "mode") {
      ...
      SystemPrompt.setSessionMode(params.sessionId, params.value)
      return {
        configOptions: configOptions(snapshot, {
  ...
  const resolveModeModel = Effect.fn("ACP.resolveModeModel")(function* (modeId: string, snapshot: Directory.Snapshot) {
    const modeConfig = MODE_MODEL_MAP[modeId]
    if (!modeConfig) return undefined
    const provider = snapshot.providers[ProviderV2.ID.make(modeConfig.providerID)]
    if (!provider || !provider.models[ModelV2.ID.make(modeConfig.modelID)]) return undefined
    return {
      model: { providerID: ProviderV2.ID.make(modeConfig.providerID), modelID: ModelV2.ID.make(modeConfig.modelID) } as ACPSession.SelectedModel,
  ...
  const setSessionMode = Effect.fn("ACP.setSessionMode")(function* (params: SetSessionModeRequest) {
    ...
    SystemPrompt.setSessionMode(params.sessionId, params.modeId)
    // Mode-model mapping: har mode ka apna model aur variant hota hai
    const modeModel = yield* resolveModeModel(params.modeId, snapshot)
    if (modeModel) {
      yield* session.setModel(params.sessionId, modeModel.model)
      yield* session.setVariant(params.sessionId, modeModel.variant)
    }
    return {}
  })
  ...
  return {
    ...
    prompt: Effect.fn("ACP.prompt")(function* (params: PromptRequest) {
      ...
      let selected = current.model ?? selectDefaultModel(snapshot)
      let variant = current.variant
      const modeId = current.modeId ?? (snapshot.availableModes.length > 0 ? snapshot.defaultModeID : undefined)
      // Mode-model mapping: ensure model matches current mode at prompt time
      if (modeId) {
        const modeModel = yield* resolveModeModel(modeId, snapshot)
        if (modeModel) {
          selected = modeModel.model
          variant = modeModel.variant
        }
      }
      if (variant === undefined) {
        variant = current.variant ?? selectVariant(snapshot, selected)
      }
      if (modeId) SystemPrompt.setSessionMode(current.id, modeId)
      ...
      if (!command) {
        ...
        if (!response || !response.info) {
          console.error("[ZOYA ACP ERROR] prompt returned empty response", { sessionID: current.id, model: `${selected.providerID}/${selected.modelID}`, providerCount: Object.keys(snapshot.providers).length })
          yield* Effect.fail(new ACPError.ServiceFailureError({ safeMessage: "ZOYA Error: Agent returned an empty response. The model may not support tool use, or the API key is invalid/limited." }));
        }
...
function makeDirectoryService(sdk: ZoyaClient) {
...
function makeUsageService(sdk: ZoyaClient) {
...
async function loadDirectorySnapshot(sdk: ZoyaClient, directory: string) {
  return ACPProfile.measure("acp.directory.load", async () => {
    ...
    // Prefer the config file model over the SDK cached model, because
    // the WebUI may have written a new model directly to the config file
    // and the backend's in-memory config cache may be stale.
    const fileConfig = readConfigDirect()
    const effectiveModel = fileConfig?.model || configResponse?.data?.model

    const defaultModelStarted = performance.now()
    const defaultModel = defaultModelFromConfig(effectiveModel, providers)
    ACPProfile.duration("acp.directory.defaultModel.resolve", defaultModelStarted, { configured: !!defaultModel, providerCount: Object.keys(providers).length, hasFileConfig: !!fileConfig, hasSdkConfig: !!configResponse?.data?.model })
    const modes = [
      { id: "fast", name: "Fast", description: "Speed-first, concise replies" },
      { id: "pro", name: "Pro", description: "Balanced depth, detailed replies" },
      { id: "expert", name: "Expert", description: "Maximum depth, comprehensive analysis" },
    ]
    const commands = [
      ...commandsData,
      ...skills
...
function defaultModelFromConfig(
  ...
  // 1️⃣  Use model explicitly configured in the CLI config
  if (configuredModel) {
    const configured = Provider.parseModel(configuredModel)

    
    const provider = providers[configured.providerID];
    if (provider) {
  
      if (provider.models[configured.modelID]) {

        return configured
      }
      console.warn("Model", configured.modelID, "not found in provider", configured.providerID, "- available:", Object.keys(provider.models))
    } else {
      console.warn("Provider", configured.providerID, "not found in loaded providers")
    }
  }

  // 2️⃣  Use the built‑in opencode provider if present
  const opencodeProvider = providers[ProviderV2.ID.make("opencode")]
  if (opencodeProvider) {
    const modelsEntries = Object.entries(opencodeProvider.models)

    // Prefer free models if available in the opencode provider
    const freeModelEntry = modelsEntries.find(([id, m]) => id.includes("free") || id.includes("nano"))
    const bestModelEntry = freeModelEntry || modelsEntries[0]
    
    if (bestModelEntry) {
        const [id, model] = bestModelEntry

        return { providerID: opencodeProvider.id, modelID: id as ModelV2.ID }
    }
  }

  // 3️⃣  Fallback — best model across ALL providers
  const allSorted = Provider.sort(
    Object.entries(providers).flatMap(([pId, p]) =>
      Object.entries(p.models).map(([mId, m]) => ({ pId, mId, m }))
    )
  )
  const best = allSorted[0]
  if (best) {
    console.warn("Falling back to best available model:", best.pId, best.mId)
    return { providerID: best.pId as ProviderV2.ID, modelID: best.mId as ModelV2.ID }
  }

  // 4️⃣  Last resort — use configured model even if not in loaded providers
  if (configuredModel) {
    const configured = Provider.parseModel(configuredModel)
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  console.error("[ZOYA ACP ERROR]", service ? `service=${service}` : "", msg, error instanceof Error ? error.stack : "")
  return new ACPError.ServiceFailureError({ safeMessage: `ZOYA service failure: ${msg}`, service })
}
