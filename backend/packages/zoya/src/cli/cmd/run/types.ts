...
import type { ZoyaClient, PermissionRequest, QuestionRequest, ToolPart } from "@zoya/sdk/v2"
import type { TuiConfig } from "@zoya/tui/config"
...
type PromptModel = Parameters<ZoyaClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ZoyaClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ZoyaClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ZoyaClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ZoyaClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ZoyaClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ZoyaClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ZoyaClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ZoyaClient["question"]["reject"]>[0]
