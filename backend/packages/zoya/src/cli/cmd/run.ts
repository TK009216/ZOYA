import type { PermissionV1 } from "@zoya/core/v1/permission"
...
//      zoya server and runs interactive mode against it.
//
...
import { createZoyaClient, type ZoyaClient, type ToolPart } from "@zoya/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<ZoyaClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run zoya with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running zoya server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'zoya')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: ZoyaClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: ZoyaClient, sessionID: string) {
      ...
      async function current(sdk: ZoyaClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ZoyaClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ZoyaClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ZoyaClient) {
        ...
        async function loop(client: ZoyaClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
